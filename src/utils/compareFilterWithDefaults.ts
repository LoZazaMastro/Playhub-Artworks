...
import { ARTWORK_PROVIDERS, MIMES, STYLES, DIMENSIONS, QUALITY_LEVELS } from '../constants';
...
const compareFilterWithDefaults = (assetType: SGDBAssetType, filters: any) => {
  ...
  return (
    ...
    (filters?.provider ? filters.provider !== ARTWORK_PROVIDERS.default : false) ||
    (filters?.providers ? filters.providers[0] !== ARTWORK_PROVIDERS.default : false) ||
    (filters?.minimumQuality ? filters.minimumQuality !== QUALITY_LEVELS.default : false) ||
    (filters?.contentType ? filters.contentType !== 'all' : false) ||
